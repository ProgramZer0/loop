...
public class GhostPlayer : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        float ghostTime = GM.getTimerSeconds() - startTimeOffset;
        if (ghostTime < 0f) return; // Ghost hasn't started yet

        Commands cmd = replayCommands[currentIndex];
        Commands nextcmd = cmd.nextCommand ?? cmd;

        if (cmd.time <= ghostTime)
        {
            if (nextcmd.time <= ghostTime)
            {
